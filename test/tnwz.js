...
const host = 'https://question.hortor.net'
...
const userForgeInfo = {
  ...
  player2: {
    uid: '11254672',
    token: 'b9c08a6c696fa67a9d6569d0a4f78454'
  }
}
...
const createSignature = (params, token) => {
  let obj = params
  if( token ){
    obj = Object.assign(params, { token })
  }
...
const login = async (player) => {
  try {
    player = userForgeInfo[player]
    let { uid, openId } = player
    let params = {
      scene: 1001,
      openId,
      playerId: uid,
    ...
    let sign = createSignature(params)
    params.sign = sign
    console.log(params)
    const res = await rp(`${host}/question/player/login`,  {form: params})

    console.log('login: ', res)
    player.token = JSON.parse(res).data.token
  } catch (error) {
