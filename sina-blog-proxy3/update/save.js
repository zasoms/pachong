    ...
    async.eachSeries(list, function(item, next) {
        ...
        db.query("SELECT * FROM `class_list` WHERE `id`=? LIMIT 1", [item.id], function(err, data) {
            ...
            if (Array.isArray(data) && data.length >= 1) {
                //分类已经存在，更新一下
                db.query(
                    "UPDATE `class_list` SET `name`=?, `url`=? WHERE `id`=?", [item.name, item.url, item.id],
                    next
                );
            } else {
                //分类不存在，添加
                db.query(
                    "INSERT INTO `class_list` (`id`, `name`, `url`) VALUES (?, ?, ?)", [item.id, item.name, item.url],
                    next
                );
            }
        });
...
exports.articleList = function(class_id, list, callback) {
    debug("保存文章列表到数据库: %d, %d", class_id, list.length);
    ...
    async.eachSeries(list, function(item, next) {
        ...
        db.query("SELECT * FROM `article_list` WHERE `id`=? AND `class_id`=? LIMIT 1", [item.id, class_id], function(err, data) {
            ...
            if (Array.isArray(data) && data.length >= 1) {
                //分类已经存在，更新一下
                db.query(
                    "UPDATE `article_list` SET `title`=?, `url`=?, `class_id`=?, `created_time`=? WHERE `id`=?",
                    [item.name, item.url, class_id, created_time, item.id, class_id],
                    next
                );
            } else {
                //分类不存在，添加
                db.query(
                    "INSERT INTO `article_list`(`id`, `title`, `url`, `class_id`, `created_time`) VALUES (?, ?, ?, ?, ?)",
                    [item.id, item.name, item.url, class_id, created_time],
                    next
                );
            }
        });
...
exports.articleDetail = function(id, tags, content, callback) {
    ...
    db.query("SELECT `id` FROM `article_detail` WHERE `id`=?", [id], function(err, data) {
        ...
        if (Array.isArray(data) && data.length >= 1) {
            //更新文章
            db.query(
                "UPDATE `article_detail` SET `tags`=?, `content`=? WHERE `id`=?",
                [tags, content, id],
                callback
            );
        }else{
            db.query(
                "INSERT INTO `article_detail` (`id`, `tags`, `content`) VALUES (?, ?, ?)",
                [id, tags, content],
                callback
            );
        }
