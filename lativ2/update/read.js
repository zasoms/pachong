  ...
  init: function(url) {
    ...
    requests.get(url)
      .end(function(err, res) {
        ...
        title = "2017诚衣lativ米格正品" + title.slice(0, title.indexOf("（"));
  ...
  getReport: function(id, callback) {
    requests.get("http://m.lativ.com/Detail/" + id)
      ...
      .end(function(err, res) {
        if (err) return console.log(err);
        var $ = cheerio.load(res.text, { decodeEntities: false });
        var $size = $("#size-report-area");
        var $try = $("#try-report-area");
        var $model = $("#model-info-area");
        $size.find('.measure-btn').remove();
        $try.find('.measure-btn').remove();

        var str = $size.html() + $try.html() + $model.html();
...
exports.getCategoryProduct = getCategoryProduct = function(callback) {
  var main = ["BABY", "KIDS"],
    mainIndex = 0,
    urls = [],
    ids = [],
